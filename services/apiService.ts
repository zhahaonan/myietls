...
export interface GrammarHintResult {
  chineseHint: string;
  contextNote: string;
}

export const generateGrammarHint = async (
  original: string,
  correction: string,
  explanation: string
): Promise<GrammarHintResult> => {
  const response = await fetch(`${API_BASE}/api/grammar-trial/generate-hint`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ original, correction, explanation }),
  });
  if (!response.ok) throw new Error(`Generate hint failed: ${response.status}`);
  return response.json();
};


export interface GrammarValidateResult {
  transcription: string;
  isCorrect: boolean;
  reason: string;
}

export const validateGrammarTrial = async (
  audioBlob: Blob,
  correction: string,
  chineseHint: string
): Promise<GrammarValidateResult> => {
  const formData = new FormData();
  formData.append("audio", audioBlob, "grammar-trial.wav");
  formData.append("correction", correction);
  formData.append("chinese_hint", chineseHint);

  const response = await fetch(`${API_BASE}/api/grammar-trial/validate`, {
    method: "POST",
    body: formData,
  });
